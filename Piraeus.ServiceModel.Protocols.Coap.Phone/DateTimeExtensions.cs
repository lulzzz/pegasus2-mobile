    ...
    internal static class DateTimeExtensions
    {
        public static byte[] Convert(this DateTime? expires, string contentType)
        {
            ...
            switch (media)
            {
                ...
                case MediaType.Xml:                
                    return expires.HasValue ? Encoding.UTF8.GetBytes(String.Format("<Expires>{0}</Expires>", expires.Value.ToString("yyyy-MM-ddTHH:MM:ssZ"))) : Encoding.UTF8.GetBytes("<Expires/>");
