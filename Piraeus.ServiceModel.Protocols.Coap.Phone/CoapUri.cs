...
namespace Piraeus.ServiceModel.Protocols.Coap
{
    ...
    public class CoapUri : Uri
    {
        ...
        public CoapUri(string uriString)
            : base(uriString.ToLower())
        {
            if (this.Scheme.ToLower() != "coaps")
            {
               
                //throw new UriFormatException("Invalid CoAP resource URI, scheme not 'coaps'.");
            }

            this.NormalizedPath = this.LocalPath.ToLower().IndexOf('/') == 0 ? this.LocalPath.ToLower().Substring(1, this.LocalPath.Length - 1) : this.LocalPath;
            ...
            string[] querySegments = this.Query.Split('&');
